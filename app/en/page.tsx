...
const fleetSpecs = {
  atto2: [
    { label: t.fleet.specs.passengers, value: "4" },
    { label: t.fleet.specs.luggage, value: "3 suitcases" },
    { label: t.fleet.specs.range, value: "312 km" },
    { label: t.fleet.specs.equipment, value: "Wifi, USB-C, A/C" },
  ],
  dolphin: [
    { label: t.fleet.specs.passengers, value: "4" },
    { label: t.fleet.specs.luggage, value: "2 suitcases" },
    { label: t.fleet.specs.range, value: "340 km" },
    { label: t.fleet.specs.equipment, value: "Wifi, USB-C, A/C" },
  ],
};

const serviceIcons = [
  <svg key="plane" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-[#C9A84C]"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.68A2 2 0 012 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 8.91a16 16 0 006.36 6.36l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  <svg key="clock" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-[#C9A84C]"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
  <svg key="brief" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-[#C9A84C]"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg>,
  <svg key="globe" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-[#C9A84C]"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>,
];
...
function FadeIn({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  const ref = useFadeIn();
  return (
    <div ref={ref} className={className} style={{ opacity: 0, transform: "translateY(24px)", transition: `opacity 0.7s ease ${delay}ms, transform 0.7s ease ${delay}ms` }}>
      {children}
    </div>
  );
}
...
function ContactForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const msg = `Hello%20Dakar%20Fleet,%0AName:%20${form.nom}%0ADate:%20${form.date}%0AType:%20${form.type}`;
  ...
  if (sent) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 rounded-[1.6rem] border border-[#C9A84C]/30 bg-white/5 p-8 text-center">
        ...
        <p className="font-title text-2xl text-[#F7F3EE]">{t.contact.form.successTitle}</p>
        <p className="text-sm text-white/60">{t.contact.form.successText}</p>
        <button onClick={() => setSent(false)} className="mt-2 text-xs text-white/40 underline underline-offset-4">{t.contact.form.newRequest}</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded-[1.6rem] border border-white/10 bg-white/5 p-6">
      <p className="text-[11px] uppercase tracking-[0.18em] text-[#C9A84C]">{t.contact.form.label}</p>
      <input type="text" placeholder={t.contact.form.name} required value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-[#F7F3EE] placeholder-white/30 outline-none transition focus:border-[#C9A84C]/50" />
      <input type="date" required value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/70 outline-none transition focus:border-[#C9A84C]/50 [color-scheme:dark]" />
      <select required value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} className="rounded-xl border border-white/10 bg-[#1a1a1a] px-4 py-3 text-sm text-white/70 outline-none transition focus:border-[#C9A84C]/50">
        <option value="">{t.contact.form.type}</option>
        {t.contact.form.types.map(type => <option key={type}>{type}</option>)}
      </select>
      <button type="submit" className="rounded-full bg-[#C9A84C] px-6 py-3 text-sm font-medium uppercase tracking-[0.1em] text-black transition hover:scale-[1.02] hover:bg-[#E8C97A]">{t.contact.form.submit}</button>
    </form>
  );
}
...
function LangSwitcher() {
  return (
    <div className="flex items-center gap-1 rounded-full border border-white/15 p-0.5 text-[0.72rem] uppercase tracking-[0.1em]">
      ...
      <span className="rounded-full bg-[#C9A84C] px-3 py-1.5 text-black font-medium">EN</span>
    </div>
  );
}
...
export default function EnPage() {
  ...
  const WA = `https://wa.me/221777796922?text=${t.whatsappMessage}`;
  const WA_PARTNER = `https://wa.me/221777796922?text=${t.whatsappPartnerMessage}`;
    ...
    <main className="bg-[#0A0A0A] text-[#F7F3EE]">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[rgba(10,10,10,0.78)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-[0.78rem] uppercase tracking-[0.12em] text-white/70 md:flex">
            ...
            <a href="#flotte" className="transition hover:text-white">{t.nav.fleet}</a>
            <a href="#partenaires" className="transition hover:text-white">{t.nav.partners}</a>
            <a href="#contact" className="transition hover:text-white">{t.nav.contact}</a>
          </nav>
          <div className="flex items-center gap-3">
            <LangSwitcher />
            <a href="tel:+221777796922" className="hidden text-sm text-[#C9A84C] transition hover:text-[#E8C97A] sm:block">+221 77 779 69 22</a>
            <a href={WA} className="rounded-full bg-[#C9A84C] px-5 py-2.5 text-[0.78rem] font-medium uppercase tracking-[0.1em] text-black transition hover:scale-[1.02] hover:bg-[#E8C97A]">{t.nav.book}</a>
            <button onClick={() => setMenuOpen(!menuOpen)} className="flex h-9 w-9 flex-col items-center justify-center gap-[5px] md:hidden" aria-label="Menu">
        ...
        <div className={`overflow-hidden transition-all duration-300 md:hidden ${menuOpen ? "max-h-80" : "max-h-0"}`}>
          <nav className="flex flex-col border-t border-white/10 bg-[rgba(10,10,10,0.97)]">
            {[["#services", t.nav.services], ["#tarifs", t.nav.pricing], ["#flotte", t.nav.fleet], ["#partenaires", t.nav.partners], ["#contact", t.nav.contact]].map(([href, label]) => (
              <a key={href} href={href} onClick={() => setMenuOpen(false)} className="border-b border-white/5 px-6 py-4 text-sm uppercase tracking-[0.12em] text-white/70 transition hover:text-white">{label}</a>
            ))}
            <a href="tel:+221777796922" className="px-6 py-4 text-sm text-[#C9A84C]">+221 77 779 69 22</a>
          </nav>
        </div>
      </header>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-14 px-6 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-24">
          <div className="flex flex-col justify-center">
            ...
            <h1 className="font-title text-5xl font-light leading-[0.98] tracking-[-0.02em] text-[#F7F3EE] sm:text-6xl lg:text-7xl">
              {t.hero.line1}<br />
              <span className="italic text-[#C9A84C]">{t.hero.line2}</span><br />
              <span className="font-semibold">{t.hero.line3}</span>
            </h1>
            <p className="mt-6 max-w-2xl text-[1.03rem] leading-8 text-white/70">
              {t.hero.body}<span className="text-white">{t.hero.bodyHighlight}</span>{t.hero.bodyEnd}
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <a href={WA} className="rounded-full bg-[#C9A84C] px-6 py-4 text-sm font-medium uppercase tracking-[0.1em] text-black transition hover:scale-[1.02] hover:bg-[#E8C97A]">{t.hero.ctaPrimary}</a>
              <a href="#services" className="rounded-full border border-white/15 px-6 py-4 text-sm uppercase tracking-[0.1em] text-white transition hover:border-[#C9A84C] hover:text-[#C9A84C]">{t.hero.ctaSecondary}</a>
            </div>
            <div className="mt-10 grid gap-3 sm:grid-cols-2">
              {t.trust.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-white/85 backdrop-blur">{item}</div>
              ))}
            </div>
          </div>
            ...
            <div className="relative w-full overflow-hidden rounded-[2rem] border border-[#C9A84C]/20 bg-white/5 p-4 backdrop-blur-xl">
              <div className="overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#111111]">
                <div className="relative h-[320px] w-full sm:h-[560px]">
                  <Image src="/atto2.png" alt="BYD Atto 2 Dakar Fleet" fill className="object-cover" priority placeholder="blur" blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==" />
      ...
      <section className="border-y border-[#2E6B4A]/30 bg-[rgba(46,107,74,0.12)]">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 py-5 text-center lg:flex-row lg:px-8 lg:text-left">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-[#F7F3EE]">{t.eco.text}<span className="font-medium text-[#4CAF7D]">{t.eco.highlight}</span>{t.eco.end}</p>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
          <FadeIn>
            <div className="max-w-3xl">
              ...
              <h2 className="mt-4 font-title text-4xl font-light leading-tight text-[#F7F3EE] sm:text-5xl">{t.services.title}</h2>
            ...
            {t.services.items.map((service, i) => (
              <FadeIn key={service.title} delay={i * 80}>
                <article className="h-full rounded-[1.8rem] border border-white/10 bg-white/5 p-6 transition duration-300 hover:-translate-y-1 hover:border-[#C9A84C]/35 hover:bg-white/[0.07]">
                  <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full border border-[#C9A84C]/20 bg-[#C9A84C]/10">{serviceIcons[i]}</div>
          ...
          <FadeIn delay={200}>
            <div className="mt-14 flex flex-col items-center gap-4 rounded-[1.8rem] border border-[#C9A84C]/15 bg-white/3 py-10 text-center">
              ...
              <a href={WA} className="rounded-full bg-[#C9A84C] px-8 py-4 text-sm font-medium uppercase tracking-[0.1em] text-black transition hover:scale-[1.02] hover:bg-[#E8C97A]">{t.services.cta}</a>
            </div>
          </FadeIn>
        </div>
      </section>
          ...
          <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
            <FadeIn>
              <div>
                ...
                <h2 className="mt-4 font-title text-4xl font-light leading-tight text-[#F7F3EE] sm:text-5xl">{t.pricing.title}</h2>
                <p className="mt-4 max-w-2xl text-lg leading-8 text-white/70">{t.pricing.subtitle}</p>
                <div className="mt-8 rounded-[1.8rem] border border-[#C9A84C]/20 bg-[#C9A84C]/10 p-6 text-white/80">{t.pricing.note}</div>
              </div>
            </FadeIn>
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
          <FadeIn>
            <div className="max-w-3xl">
              ...
              <h2 className="mt-4 font-title text-4xl font-light leading-tight text-[#F7F3EE] sm:text-5xl">{t.fleet.title}</h2>
              <p className="mt-4 text-lg leading-8 text-white/70">{t.fleet.subtitle}</p>
            </div>
          </FadeIn>
          <div className="mt-14 grid gap-8 lg:grid-cols-2">
            {(["atto2", "dolphin"] as const).map((key, i) => (
              <FadeIn key={key} delay={i * 100}>
                <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5">
                  <div className="relative h-[280px] w-full sm:h-[390px]">
                    <Image src={key === "atto2" ? "/atto2.png" : "/dolphin.png"} alt={t.fleet.vehicles[i].name} fill className="object-cover" placeholder="blur" blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==" />
                  </div>
                  <div className="p-6">
                    <p className="text-[11px] uppercase tracking-[0.18em] text-white/45">{t.fleet.vehicles[i].tag}</p>
                    <h3 className="mt-2 font-title text-3xl text-[#F7F3EE]">{t.fleet.vehicles[i].name}</h3>
                    <p className="mt-3 leading-7 text-white/70">{t.fleet.vehicles[i].description}</p>
                    <div className="mt-5 grid grid-cols-2 gap-2 border-t border-white/8 pt-5">
                      {fleetSpecs[key].map((spec) => (
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_1fr]">
            <FadeIn>
              <div>
                ...
                <h2 className="mt-4 font-title text-4xl font-light leading-tight text-[#F7F3EE] sm:text-5xl">{t.partners.title}</h2>
                <p className="mt-4 text-lg leading-8 text-white/70">{t.partners.subtitle}</p>
                <a href={WA_PARTNER} className="mt-8 inline-block rounded-full border border-[#C9A84C]/40 px-6 py-3 text-sm uppercase tracking-[0.1em] text-[#C9A84C] transition hover:border-[#C9A84C] hover:bg-[#C9A84C]/10">{t.partners.cta}</a>
              </div>
            </FadeIn>
              ...
              {t.partners.items.map((point, i) => (
                <FadeIn key={point} delay={i * 70}>
                  <div className="flex items-start gap-3 rounded-[1.6rem] border border-white/10 bg-white/5 px-5 py-5 text-white/80">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 h-4 w-4 shrink-0"><polyline points="20 6 9 17 4 12"/></svg>
                    {point}
                  </div>
                </FadeIn>
              ))}
      ...
      <footer className="border-t border-white/10 bg-black">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 text-white/70 lg:grid-cols-3 lg:px-8">
          <div>
            <div className="font-title text-2xl tracking-[0.22em] text-[#C9A84C]">DAKAR<span className="text-[#F7F3EE]">FLEET</span></div>
          ...
          <div>
            ...
            <div className="mt-4 space-y-3">
              {t.footer.services.map(s => <p key={s}>{s}</p>)}
            </div>
          </div>
        </div>
      </footer>
      ...
      {/* ── WHATSAPP FLOAT ── */}
      <a href={WA} className="fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-4 text-sm font-semibold text-white shadow-lg shadow-green-900/30 transition hover:scale-105">
